    ...
    describe(`ROUTER = ${routerName}`, () => {
      ...
      describe(`.fetch({ method = 'GET', url })`, () => {
        ...
        it('allows missing handler later in flow with "all" channel', async () => {
          ...
          const router2 = Router({ base: '' })

          router2.get('/foo', matchHandler)
          router1.use('/nested/*', router2).all('*', missingHandler)
        ...
        it('can pull route params from the basepath as well', async () => {
          const router = Router({ base: '' })
          const handler = vi.fn((req) => req.params)
          router.get('/:collection/:id', handler)
      ...
      describe('NESTING', () => {
        it('can handle legacy nested routers (with explicit base path)', async () => {
          ...
          router1.get('/nested/*', router2)
          router2.get('/nested', handler3)
          router2.get('/nested/bar/:id?', handler2)
        ...
        it('can nest with route params on the nested route if given router.fetch and base path', async () => {
          const child = Router({ base: '' }).get('/', () => 'child')
          const parent = Router()
            .get('/', () => 'parent')
            .use('/child/:bar/*', child)
